<template>
      ...
      <Card
        ...
        :class="{
          'border-2 border-black': selectedGifts.some(item => item.id === freeShipping.id) || !isDisabled,
          'border-2 border-white': !selectedGifts.some(item => item.id === freeShipping.id) || isDisabled,
        }"
        @click="toggleSelectGifts(freeShipping)"
      >
        <template #header>
          <div class="w-full h-24 mb-2 card-image relative">
            <img class="object-contain h-full mx-auto" alt="product image" :src="freeShipping.image" />
          </div>
        </template>
  
        <template #title>
          <h2 class="text-black font-almarai text-[14px] font-medium leading-[14.4px] tracking-[-0.02em] text-center">
